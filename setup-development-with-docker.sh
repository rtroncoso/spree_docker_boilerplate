...
# 5: Install spree and all of it's dependencies
docker-compose run --rm web rails g spree:install --user_class=Spree::User
docker-compose run --rm web rails g spree:auth:install
docker-compose run --rm web rails g spree_gateway:install

# 6: Stop all the running containers:
docker-compose stop
